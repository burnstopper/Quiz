...
async def update_template(template_id: int, template_in: TemplateUpdate, db: AsyncSession = Depends(get_db)):
    ...
    error: None | str = await crud.update_template(db=db, template_id=template_id, template_in=template_in)
    if error is not None:
        raise HTTPException(status_code=409, detail=error)
...
async def get_template_by_id(template_id: int, db: AsyncSession = Depends(get_db)) -> Template:
    """
    Get template by id
    """
